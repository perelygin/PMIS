...
$this->title = 'Сводный отчет по событиям абонемента.';
...
<div class="vw-report1-index">
    ...
    echo $this->render('_search_report3', ['model' => $searchModel]); ?>
<!--
	 <style type="text/css">
	   TABLE {
		  ...
		  <?php 
		    ...
		    if(!empty($dataProvider)){
				...
				$BR = BusinessRequests::findOne($i);
				 echo '<tr><td> BR-'.$dataProvider[0]['BRNumber'].' '.$dataProvider[0]['BRName'].'</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>';	 
		    ...
		    foreach($dataProvider as $dp_str){
				...
				$color1 ='';
				 if($i <> $dp_str->idBr){  //
					  ...
					  echo '<tr><td> BR-'.$dp_str->BRNumber.' '.$dp_str->BRName.'</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>';
					  echo '<tr>
					  <td></td>
